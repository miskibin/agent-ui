...
/* -------------------------------------------------------------------------- */
/*                        the loop around the translator                       */
/* -------------------------------------------------------------------------- */

/**
 * pi is stood in for by a script that replays a fixture of its `--mode rpc`
 * records: `resolvePiCommand` runs a `.js` `binPath` with the Node we are
 * already inside, so this drives the real spawn, the real framing and the real
 * teardown without the CLI. It never reads stdin — the commands `runPiAgent`
 * writes there are answered by the fixture, the way the real binary would.
 *
...
writeFileSync(
  ...
  `const fs = require("node:fs")\n` +
    // stdin is a pipe nobody reads here; swallowing its errors keeps the
    // stub's exit code the only thing the test is reading.
...
function runAgent(signal?: AbortSignal) {
  return runPiAgent({
    prompt: "hi",
...
async function collect(signal?: AbortSignal) {
  ...
  for await (const event of runAgent(signal)) events.push(event)
  return events
}

/** The `get_state` answer the session id is read out of, as a plain record. */
const stateLine = {
  id: "state",
  type: "response",
  command: "get_state",
...
test("a tool pi never finished is closed when the process exits non-zero", async () => {
  stage([stateLine, startBash], { exit: 2 })

  const events = await collect()
  assert.deepEqual(events[0], { type: "session", sessionId: SESSION })
...
test("a tool that completed is left alone and the turn still finishes", async () => {
  stage([
    ...
    {
      type: "message_end",
      message: { role: "assistant", usage: { input: 10, output: 4 } },
    },
    { type: "agent_settled" },
  ])
...
test("stopping the turn closes the rows pi left open", async () => {
  stage([stateLine, startBash], { holdMs: 30_000 })

  const controller = new AbortController()
  const events: AgentStreamEvent[] = []
  for await (const event of runAgent(controller.signal)) {
    events.push(event)
