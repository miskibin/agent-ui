...
A fast, local-first chat app for coding agents. One interface, swappable backends: the local `cursor-agent` CLI, any model served by [Ollama](https://ollama.com), or a scripted mock — with Claude Code and OpenCode adapters on the roadmap. Built entirely on [miskibin/chat-components](https://github.com/miskibin/chat-components), the shadcn/ui registry of agent-grade chat primitives.
...
| Dark mode | Settings |
| --- | --- |
| ![Dark mode](.github/screenshots/chat-dark.png) | ![Settings](.github/screenshots/settings.png) |
...
Production is a self-contained standalone server — fast cold start, no `node_modules` on the target machine:
